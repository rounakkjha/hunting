...
import type { TargetCompany, CompanyContact } from '../App';
import CompanyAutocomplete from './CompanyAutocomplete';

interface TargetCompaniesProps {
  companies: TargetCompany[];
  onAdd: (company: string, role?: string, date?: string) => void;
...
export default function TargetCompanies({
  ...
  const [pendingCompany, setPendingCompany] = useState<{ company: string; role?: string; date: string } | null>(null);
  ...
  const handleAddCompany = (e: React.FormEvent) => {
    ...
    onAdd(newCompany.trim(), newRole.trim() || undefined, newDate);
    setNewCompany('');
    setNewRole('');
          ...
          {showAddForm && (
            ...
            >
              ...
              <CompanyAutocomplete
                ...
                required
              />
              <label className="block text-sm font-medium text-foreground/80 mb-1">Role (optional)</label>
              <input
                type="text"
                value={newRole}
                          ...
                          <button
                            onClick={() => {
                              if (editCompanyName.trim()) {
                                onUpdateCompany(company.id, { company: editCompanyName.trim(), role: editCompanyRole.trim() || undefined });
                              }
          ...
          <div className="bg-card rounded-2xl border border-border/60 shadow-2xl w-full max-w-md p-6 animate-scale-in">
            ...
            <div className="flex gap-3">
              ...
              <button
                onClick={() => {
                  if (pendingCompany) {
                    onAdd(pendingCompany.company, pendingCompany.role, pendingCompany.date);
                  }
                  setShowDuplicateWarning(false);
                  setPendingCompany(null);
                  setNewCompany('');
